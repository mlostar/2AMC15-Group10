...
register_env('FloorCleaning', lambda env_config: Discretiser(FloorCleaning({"robot": robot, "grid": grid}), n_splits=4))
...
env = Discretiser(FloorCleaning({"robot": robot, "grid": grid}), n_splits=4)
for e in range(10):
    obs = env.reset()
    env.render()
    ...
    for s in range(1000):
        ...
        env.render()
        print(f"move: {move*360/2*np.pi}, reward: {reward}")
