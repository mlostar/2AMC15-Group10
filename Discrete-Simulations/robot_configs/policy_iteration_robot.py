...
def robot_epoch(robot, gamma=0.3, theta=0.01):
    """
    Run an epoch of the policy iteration robot.
    """
    # Initialization
    ...
    # Initialize equal probability policy
    policy = 1 / 4 * np.ones((robot.grid.n_cols, robot.grid.n_rows, 4))

    policy_iteration(robot, V, policy)
...
def policy_iteration(robot, V, policy):
    """
    Perform policy iteration until the optimal policy is found.
    """
    policy_stable = False
...
def policy_evaluation(robot, V, policy):
    """
    Perform the policy iteration to the given V.
    """
    ...
    while True:
        ...
        for x in range(1, n_cols - 1):
            for y in range(1, n_rows - 1):
                ...
                v = 0
                v += calc_action_val('n', current_pos, grid_cells,
                                     old_V, gamma, robot.p_move, policy, True)
                v += calc_action_val('e', current_pos, grid_cells,
                                     old_V, gamma, robot.p_move, policy, True)
                v += calc_action_val('s', current_pos, grid_cells,
                                     old_V, gamma, robot.p_move, policy, True)
                v += calc_action_val('w', current_pos, grid_cells,
                                     old_V, gamma, robot.p_move, policy, True)
...
def policy_improvement(robot, V, policy):
    """
    Perform the policy improvement stage to update the given policy.
    """
    ...
    for x in range(1, n_cols - 1):
        for y in range(1, n_rows - 1):
            ...
            action_values = [calc_action_val('n', current_pos, grid_cells, V, gamma, robot.p_move),
                             calc_action_val(
                                 'e', current_pos, grid_cells, V, gamma, robot.p_move),
                             calc_action_val(
                                 's', current_pos, grid_cells, V, gamma, robot.p_move),
...
def calc_action_val(action_orientation, pos, grid_cells, V, gamma, random_move_p, policy=np.empty, use_policy=False):
    ...
    if action_orientation == 'n':
        r_move_n = policy_n * ((1-random_move_p)+(random_move_p/4)) * \
            (grid_cells[x, y - 1] + gamma * V[x, y - 1])
        r_move_s = policy_s * (random_move_p/4) * \
            (grid_cells[x, y+1]+gamma*V[x, y+1])
        r_move_e = policy_e * (random_move_p/4) * \
            (grid_cells[x+1, y]+gamma*V[x+1, y])
        r_move_w = policy_w * (random_move_p/4) * \
            (grid_cells[x-1, y]+gamma*V[x-1, y])
    elif action_orientation == 's':
        r_move_n = policy_n * (random_move_p / 4) * \
            (grid_cells[x, y - 1] + gamma * V[x, y - 1])
        r_move_s = policy_s * ((1 - random_move_p) + (random_move_p / 4)) * \
            (grid_cells[x, y + 1] + gamma * V[x, y + 1])
        r_move_e = policy_e * (random_move_p / 4) * \
            (grid_cells[x + 1, y] + gamma * V[x + 1, y])
        r_move_w = policy_w * (random_move_p / 4) * \
        ...
        r_move_s = policy_s * (random_move_p / 4) * \
            (grid_cells[x, y + 1] + gamma * V[x, y + 1])
        r_move_e = policy_e * ((1 - random_move_p) + (random_move_p / 4)) * \
            (grid_cells[x + 1, y] + gamma * V[x + 1, y])
        r_move_w = policy_w * (random_move_p / 4) * \
            (grid_cells[x - 1, y] + gamma * V[x - 1, y])
        ...
        r_move_e = policy_e * (random_move_p / 4) * \
            (grid_cells[x + 1, y] + gamma * V[x + 1, y])
        r_move_w = policy_w * ((1 - random_move_p) + (random_move_p / 4)) * \
            (grid_cells[x - 1, y] + gamma * V[x - 1, y])
